...
class Bi_Lstm_model(object):
	...
	def run(self):
		...
		with tf.name_scope('lstm'):
			...
			fw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.hidden_dim, forget_bias=1.0)
			bw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.hidden_dim, forget_bias=1.0)
			(fw_outputs, bw_outputs),_ = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, embedding_inputs, dtype=tf.float32)
